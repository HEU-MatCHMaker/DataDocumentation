...
At the current stage, it is not actual datasets we want to document, but the type of datasets that the computation takes as input and output.
...
The documentation of datasets will be based on this vocabulary, but may also extended with additional concepts needed for describing materials science datasets.
...
Based on the asserted `hasInput` and `hasOutput` relations, a reasoner can now infer the `isImpactedBy` and `processedFrom` relations shown in the [introductory section on documenting workflows](#documenting-the-actual-datasets-and-executed-workflows).
With these inferred relations, you get full provenance of computations (processes) and datasets (objects), respectively.
